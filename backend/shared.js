...
export function sharedStore(ctx, path, body = {}, params = new URLSearchParams()) {
  ...
  if (path === '/internal/shared/state' || path === '/internal/shared/inbox') {
    ...
    const result = {messages:selected.filter(r=>r.kind!=='briefing').map(entry),posts:selected.filter(r=>r.kind==='briefing').map(entry),
      ...
      lastSuccessfulCheck:rows("SELECT value FROM shared_meta WHERE key='checked'")[0]?.value || null,
      mode:'direct-cloudflare', publisher:{ok:true,source:'Cloudflare'}};
    if (path.endsWith('/inbox')) {
      ...
      result.lastSuccessfulCheck=new Date().toISOString();
      sql.exec("INSERT OR REPLACE INTO shared_meta VALUES('checked',?)",result.lastSuccessfulCheck);
    }
    return json(result);
  }
  ...
  return ctx.storage.transactionSync(() => {
    ...
    const prior=kind==='reply'?rows('SELECT * FROM shared_entries WHERE reply_to=?',body.replyTo)[0]:existing;
