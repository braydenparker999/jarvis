...
export default {
  async fetch(request, env) {
    ...
    if(path==='/health' && request.method==='GET') return reply({ok:true,mode:'direct-cloudflare',version:5,assistantConfigured:!!(env.GITHUB_CLIENT_ID&&env.GITHUB_CLIENT_SECRET)});
...
export class Hub {
  ...
  async fetch(request){
    const path=new URL(request.url).pathname;
    if(path.startsWith('/internal/shared/'))return sharedStore(this.ctx,path,request.method==='POST'?await request.json():{},new URL(request.url).searchParams);
