...
function drawHome() {
  ...
  $('content').innerHTML = `<section class="greeting"><p class="eyebrow">${escape(date)}</p><h1>${greeting}, Brayden<span class="mint">.</span></h1><p class="subheading">A space for your thoughts, updates, and everyday life.</p></section><div class="home-grid"><section class="card feature"><div class="card-top"><span class="eyebrow">JARVIS</span><span class="pill">${API_ORIGIN?'RECEIPT MODE':'DRAFT MODE'}</span></div><div class="orb" aria-hidden="true"><span></span></div><h2>What's on your mind?</h2><p>Keep a thought. Start a conversation.</p><a class="primary" href="/jarvis/" data-route="chat">Open Jarvis ${svg('arrow')}</a><small>${API_ORIGIN?'Automatic delivery receipts. AI replies are not connected yet.':'Drafts save here while cloud messaging is being connected.'}</small></section><section class="card board-preview"><div class="card-top"><span class="eyebrow">DAILY BOARD</span>${svg('board')}</div><h2>${latest?escape(latest.title):'Room for the day ahead.'}</h2><p class="preview-body">${latest?escape(latest.body):'Your updates and notes, collected in one place. Add your first entry to get started.'}</p>${latest?`<span class="entry-meta">${time(latest.createdAt)} · ${latest.saved?'Cloud saved':'On this device'}</span>`:''}<a class="text-link" href="/daily-board/" data-route="board">${latest?'View your board':'Write your first entry'} ${svg('arrow')}</a></section><section class="card activity"><div><span class="eyebrow">AT A GLANCE</span><h2>Your hub, so far.</h2></div><div class="stats"><div><strong>${state.posts.length}</strong><span>Board entries</span></div><div><strong>${state.messages.filter(m=>m.role==='user').length}</strong><span>Messages</span></div><div><strong>${state.outbox.length}</strong><span>Awaiting sync</span></div></div></section></div>`;
}
function drawChat() {
  $('content').innerHTML = `<section class="page-heading"><div><p class="eyebrow">YOUR CONVERSATION SPACE</p><h1>Jarvis<span class="mint">.</span></h1></div><span class="pill">${API_ORIGIN?'RECEIPT MODE':'DRAFT MODE'}</span></section><section class="chat-panel"><div class="chat-notice">${API_ORIGIN?'Messages receive an automatic delivery receipt. AI replies are not connected yet.':'Cloud messaging is not connected yet. Your drafts stay on this device and have not been sent.'}</div><div class="messages" id="messages" aria-label="Conversation">${state.messages.length?state.messages.map(messageMarkup).join(''):`<div class="chat-empty"><div class="orb small" aria-hidden="true"><span></span></div><h2>A place to pick up your thoughts.</h2><p>Write a message below. ${API_ORIGIN?'Jarvis will confirm when it is saved.':'It will wait here until cloud messaging is ready.'}</p></div>`}</div><form class="composer" id="message-form"><label class="sr-only" for="message-text">Message Jarvis</label><textarea id="message-text" rows="2" maxlength="4000" placeholder="Write something…" required>${escape(state.composer || '')}</textarea><div class="composer-bottom"><span>${API_ORIGIN?'Enter to send · Shift + Enter for a new line':'Saved on this device · not sent'}</span><button class="primary" type="submit" id="send-message">${API_ORIGIN?'Send':'Save draft'} ${svg('send')}</button></div></form></section>`;
...
function messageMarkup(m) {
  return `<article class="message-row ${m.role==='user'?'outgoing':'incoming'}"><span class="message-author">${m.role==='user'?'YOU':'JARVIS · AUTOMATIC RECEIPT'}</span><p class="bubble">${escape(m.body)}</p><span class="message-time">${time(m.createdAt)} · ${m.saved?'Cloud saved':'Not sent · on this device'}</span></article>`;
}
...
function drawBoard() {
  $('content').innerHTML = `<section class="page-heading"><div><p class="eyebrow">THOUGHTS, NOTES & UPDATES</p><h1>Daily Board<span class="mint">.</span></h1><p class="subheading">Keep what matters today.</p></div><button class="primary" id="new-entry">${svg('plus')} New entry</button></section><div class="board-list">${state.posts.length?[...state.posts].reverse().map(p=>`<article class="card board-entry"><div class="card-top"><span class="eyebrow">${time(p.createdAt)}</span><span class="pill">${p.saved?'CLOUD SAVED':'ON THIS DEVICE'}</span></div><h2>${escape(p.title)}</h2><p>${escape(p.body)}</p></article>`).join(''):`<section class="card empty-board"><span class="empty-icon">${svg('board')}</span><h2>Your first page starts here.</h2><p>A thought worth keeping, a note for tomorrow, or a small win from today.</p><button class="text-link" id="first-entry">Write an entry ${svg('arrow')}</button></section>`}</div><p class="board-note">${API_ORIGIN?'Entries sync with your connected workspace.':'Entries are saved on this device. Cloud sync and scheduled updates are not connected yet.'}</p>`;
