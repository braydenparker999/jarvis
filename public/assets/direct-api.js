...
export function createDirectApi(fetcher=fetch,origin=API_ORIGIN) {
  ...
  async function load() {
    ...
    do {
      ...
      if(state.mode!=='direct-cloudflare'||!Array.isArray(state.messages)||!Array.isArray(state.posts))throw Error('Cloud inbox returned invalid data. Your drafts are safe.');
