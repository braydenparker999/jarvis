  (()=>{'use strict';
    ...
    const state={addons:AstraCollections.restoreAddons(store.get('addons',DEFAULT_ADDONS),DEFAULT_ADDONS),manifests:new Map(),catalogCache:new Map(),metaCache:new Map(),catalogRegistry:[],homeLayout:store.get('homeLayout',AstraCatalogs.defaults()),currentPage:'home',currentMeta:null,currentVideo:null,currentStreams:[],homeItems:[],discoverItems:[],discoverVisible:DISCOVER_BATCH,libraryVisible:LIBRARY_BATCH,libraryView:'all',libraryQuery:'',libraryType:'all',librarySort:'recent',recentSearches:AstraCollections.recent(store.get('recentSearches',[])),library:AstraCollections.restoreLibrary(store.get('library',{})),settings:AstraPlayback.settings.migrate(store.get('settings',{})),addonHealth:AstraDiscovery.normalizeHealth(store.get('addonHealth',null)),healthChecking:new Set(),healthRun:0,briefing:{seen:new Set()},discover:{type:'all',sector:null,sectorLabel:'',addon:'all',catalog:'all',genre:'all'},discoverSources:[],query:'',settingsRoute:'root',searchToken:0,searchSequence:0,searchRun:null,detailBrowser:null};
    ...
    function mediaImage(url,options={}){
      ...
      return `<span class="art-loader" aria-hidden="true"></span><img class="media-image" ${priority} decoding="async" referrerpolicy="no-referrer" src="${esc(candidates[0])}" data-art-fallbacks="${esc(fallbacks)}" alt="">`;
    }
    ...
    function bindArtwork(root=document){
      $$('.media-image:not([data-art-bound])',root).forEach(image=>{
        image.dataset.artBound='true';
